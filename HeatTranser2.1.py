...
mkpath="C:\\Users\\你的用户名\\Desktop\\HeatTransfer\\2.1\\"+str(nowtime)+"\\"
# 调用函数
mkdir(mkpath)
filenames=[]
...
def update(Alpha,dt,dx):
    ...
    for i in range(len(A)):
        if A[i].boundarycondition==0:
            A[i].T+=A[i].dT
            A[i].dT=dt*Alpha*(qdotDividedByk+(A[i-1].T+A[i+1].T-2*A[i].T)/(dx**2))
...
f.write("dx:"+str(dx))
f.write("\n")
f.write("qdotDividedByk:"+str(qdotDividedByk))
f.write("\n")
